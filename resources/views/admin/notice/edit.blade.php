...
@section('contenido')
<h1 class="text-3xl font-bold text-center uppercase">Editar Aviso</h1>
<div class="mt-5">
    <div class="w-[600px] mx-auto sm:px-6 lg:px-8">
        <div class="overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200 flex flex-col justify-center w-full">
                ...
                <form action="{{ route('notices.update', $notice->slug) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    @else
                        <div class="mb-4 w-full">
                            ...
                            >
                                ...
                                @foreach($gyms as $gym)
                                    <option value="{{ $gym->id }}" @if($gym->id == $notice->id_gym) selected @endif>{{ $gym->name }}</option>
                                @endforeach
                            </select>
                            @error('id_gym')
                    ...
                    <div class="mb-4 w-full">
                        ...
                        >
                            ...
                            <option value="1" @if($notice->isActive) selected @endif>Activo</option>
                            <option value="0" @if(!$notice->isActive) selected @endif>Inactivo</option>
                        </select>
                        @error('isActive')
                            <div class="text-red-500 text-xs mt-1">{{ $message }}</div>
                        @enderror
